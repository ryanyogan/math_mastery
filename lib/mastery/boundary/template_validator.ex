defmodule Mastery.Boundary.TemplateValidator do
  ...
  def validate_instructions(instructions) when is_binary(instructions),
    do: :ok

  def validate_instructions(_instructions),
    do: {:error, "must be a binary"}
  ...
  def validate_generator({name, generator})
    ...
    check(generator != [], {:error, "cannot be empty"})
  end
  ...
  def validate_generator(_generator),
    do: {:error, "must be a string to list, or a function pair"}

  def validate_checker(checker) when is_function(checker, 2), do: :ok

  def validate_checker(_checker),
    do: {:error, "must be an arity/2 function"}
end
